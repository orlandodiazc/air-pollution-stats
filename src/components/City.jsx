...
export default function City({ cityLocation }) {
  ...
  const { data: cityAirPollution, isSuccess: isPollutionSuccess } =
    useGetCityPollutionQuery({
      lat,
      lon,
    });
  const { main: airQuality } = isPollutionSuccess
    ? cityAirPollution.list[0]
    : {};
    ...
    return (
      <li className="border border-opacity-40 border-gray-200">
        <div className="px-3 py-1">
          <Link
            state={{ cityAirPollution, cityLocation }}
            to={`/details/${name}`}
          >
